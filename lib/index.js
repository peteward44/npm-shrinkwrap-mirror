...
function getFilesToDownload( swjson, newRootUrl, outdir, force, output ) {
	...
	for ( var name in swjson.dependencies ) {
		if ( swjson.dependencies.hasOwnProperty( name ) ) {
			...
			try {
				if ( dep.resolved ) {
					var matches = dep.resolved.match( /\.tgz$/i );
					if ( matches ) {
...
function start( options ) {
	...
	if ( !fs.existsSync( options.shrinkwrap ) ) {
		if ( !options.quiet ) {
			console.error( "Shrinkwrap file '" + options.shrinkwrap + "' not found! Run 'npm shrinkwrap' in your project before running npm-shrinkwrap-mirror" );
		}
